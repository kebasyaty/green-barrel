...
fn test_model_number_i64_fields() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        1_i64,
        map_to_wigets.get("radio").unwrap().value.parse::<i64>()?
    );
    ...
    assert_eq!(
        105_i64,
        map_to_wigets.get("number").unwrap().value.parse::<i64>()?
    );
    // range
    let map_to_wigets = app_name::TestModel::to_wig()?;
    assert_eq!(
        5_i64,
        map_to_wigets.get("range").unwrap().value.parse::<i64>()?
    );
    let map_to_wigets = result_2.to_wig();
    assert_eq!(
        9_i64,
    ...
    assert_eq!(
        3_i64,
        map_to_wigets.get("hidden").unwrap().value.parse::<i64>()?
    );
    let map_to_wigets = result_2.to_wig();
    assert_eq!(
        11_i64,
